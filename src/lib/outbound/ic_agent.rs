...
impl TransactionRepository for IcAgentTransactionRepository {
    async fn create_transaction(&self, req: &CreateTransactionRequest) -> Result<Transaction, CreateTransactionError> {
        ...
        customer.country_code.parse::<u16>().map_err(|e| anyhow!("Could not parse country code"))?,
        MobileNumber::new(customer.mobile_number.parse::<u64>().map_err(|e| anyhow!("Could not parse country code"))?)?,
